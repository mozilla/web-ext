...
describe('firefox', () => {
  describe('run', () => {
    ...
    it('starts the remote debugger on a discovered port', () => {
      ...
      return runFirefox({ fxRunner: runner, findRemotePort }).then(() => {
        sinon.assert.called(runner);
    ...
    it('sets up a Firefox process environment', () => {
      ...
      return runFirefox({ fxRunner: runner }).then(() => {
    ...
    it('passes a custom Firefox binary when specified', () => {
      ...
      return runFirefox({ fxRunner: runner, firefoxBinary }).then(() => {
        sinon.assert.called(runner);
    ...
    it('logs link to debugging docs', async () => {
      ...
      assert.notOk(
        consoleStream.capturedMessages.find((msg) =>
          msg.includes(expectedMessage)
        )
      );
      assert.notOk(
        consoleStream.capturedMessages.find((msg) =>
          msg.includes(expectedURLToDocs)
        )
      );
      ...
      const foundMessage = consoleStream.capturedMessages.find((msg) =>
        msg.includes(expectedMessage)
      );
      const foundDocURL = consoleStream.capturedMessages.find((msg) =>
        msg.includes(expectedURLToDocs)
      );
  ...
  describe('copyProfile', () => {
    ...
    it('requires a valid profile directory', () => {
      ...
      // user directory.
      const copyFromUserProfile = sinon.spy((config, cb) =>
        cb(new Error('simulated: could not find profile'))
      );

      return firefox
        .copyProfile('/dev/null/non_existent_path', {
          copyFromUserProfile,
          configureThisProfile: (profile) => Promise.resolve(profile),
        })
        .then(makeSureItFails())
    ...
    it('can copy a profile by name', () => {
      ...
      const copyFromUserProfile = sinon.spy((config, callback) =>
        callback(null, profileToCopy)
      );
      ...
      return firefox
        ...
        .then((profile) => {
          ...
          assert.equal(
            profile.defaultPreferences.thing,
            profileToCopy.defaultPreferences.thing
          );
        });
    });

    it('configures the copied profile', () =>
      withBaseProfile((baseProfile) => {
        const app = 'fennec';
        const configureThisProfile = sinon.spy((profile) =>
  ...
  describe('isDefaultProfile', () => {
    it('detects common Firefox default profiles specified by name', async () => {
    ...
    it('allows profile name if it is not listed as default in profiles.ini', async () => {
      return withTempDir(async (tmpDir) => {
        ...
        assert.equal(isDefault, true, 'Manually configured default profile');

        const isNotDefault = await firefox.isDefaultProfile(
          'unkown-profile-name',
          FakeProfileFinder
        );
        assert.equal(isNotDefault, false, 'Unknown profile name');
      });
    });

    it('allows profile path if it is not listed as default in profiles.ini', async () => {
      return withTempDir(async (tmpDir) => {
        const profilesDirPath = tmpDir.path();
        const FakeProfileFinder = createFakeProfileFinder(profilesDirPath);
        ...
        assert.equal(isFirefoxDefaultPath, true, 'Firefox default profile');
        ...
        assert.equal(isNotDefault, false, 'Unknown profile path');
      });
    });

    it('allows profile path if there is no profiles.ini file', async () => {
      return withTempDir(async (tmpDir) => {
        const profilesDirPath = tmpDir.path();
        const FakeProfileFinder = createFakeProfileFinder(profilesDirPath);
    ...
    it('rejects on any unexpected error while looking for profiles.ini', async () => {
      return withTempDir(async (tmpDir) => {
        const profilesDirPath = tmpDir.path();
        const FakeProfileFinder = createFakeProfileFinder(profilesDirPath);
  ...
  describe('createProfile', () => {
    ...
    it('configures a profile', () => {
      ...
      const app = 'fennec';
      return firefox
        .createProfile({ app, configureThisProfile })
        .then((profile) => {
          sinon.assert.called(configureThisProfile);
          sinon.assert.calledWith(configureThisProfile, profile);
          assert.equal(configureThisProfile.firstCall.args[1].app, app);
  ...
  describe('useProfile', () => {
    it('rejects to a UsageError when used on a default Firefox profile', async () => {
      const configureThisProfile = sinon.spy((profile) =>
        Promise.resolve(profile)
      );
      const isFirefoxDefaultProfile = sinon.spy(() => Promise.resolve(true));
      let exception;
    ...
    it('rejects to a UsageError when profile is not found', async () => {
      const fakeGetProfilePath = sinon.spy(() => Promise.resolve(false));
      const createProfileFinder = () => {
        return fakeGetProfilePath;
      };
      const isFirefoxDefaultProfile = sinon.spy(() => Promise.resolve(false));
    ...
    it('resolves to a FirefoxProfile instance', () =>
      withBaseProfile(async (baseProfile) => {
    ...
    it('looks for profile path if passed a name', () =>
      withBaseProfile(async (baseProfile) => {
        ...
        sinon.assert.calledWith(fakeGetProfilePath, sinon.match('profileName'));
      }));

    it('checks if named profile is default', () =>
      withBaseProfile(async (baseProfile) => {
        const createProfileFinder = () => {
    ...
    it('configures a profile', () =>
      withBaseProfile((baseProfile) => {
        ...
        return firefox
          .useProfile(profilePath, { configureThisProfile })
          .then((profile) => {
            sinon.assert.called(configureThisProfile);
            sinon.assert.calledWith(configureThisProfile, profile);
          });
  ...
  describe('defaultCreateProfileFinder', () => {
    ...
    it('returns a finder that resolves a profile name', async () => {
      ...
      const { fakeReadProfiles, fakeGetPath, FxProfile, userDirectoryPath } =
      ...
      assert.equal(
        await profileFinder('someName'),
        expectedResolvedProfilePath,
    ...
    it('returns a finder that resolves undefined for no profiles.ini', async () => {
      const { fakeReadProfiles, fakeGetPath, FxProfile, userDirectoryPath } =
        prepareProfileFinderTest({
          readProfiles(cb) {
    ...
    it('returns a finder that throws unexpected errors', async () => {
      const { fakeReadProfiles, fakeGetPath, FxProfile, userDirectoryPath } =
        prepareProfileFinderTest({
          readProfiles(cb) {
  ...
  describe('configureProfile', () => {
    ...
    it('resolves with a profile', () =>
      withTempProfile((profile) => {
        const fakePrefGetter = sinon.stub().returns({});
        return firefox
          .configureProfile(profile, { getPrefs: fakePrefGetter })
          .then((configuredProfile) => {
            assert.instanceOf(configuredProfile, FirefoxProfile);
          });
      }));

    it('sets Firefox preferences', () =>
      withTempProfile((profile) => {
        const fakePrefGetter = sinon.stub().returns({});
        return firefox
          .configureProfile(profile, { getPrefs: fakePrefGetter })
    ...
    it('writes custom preferences', () =>
      withTempProfile((profile) => {
        const customPrefs = { 'extensions.checkCompatibility.nightly': true };
        return firefox
          .configureProfile(profile, { customPrefs })
          .then((configuredProfile) => {
            return fs.readFile(path.join(configuredProfile.path(), 'user.js'));
          })
          .then((prefFile) => {
  ...
  describe('installExtension', () => {
    ...
    it('installs an extension file into a profile', () =>
      setUp((data) => {
        return installBasicExt(data)
          .then(() => fs.readdir(data.profile.extensionsDir))
          .then((files) => {
            assert.deepEqual(files, ['basic-manifest@web-ext-test-suite.xpi']);
          });
      }));

    it('requires a manifest ID', () =>
      setUp((data) => {
        return firefox
          .installExtension({
          ...
          .catch(
            onlyInstancesOf(UsageError, (error) => {
              assert.match(error.message, /explicit extension ID is required/);
            })
          );
      }));

    it('can install the extension as a proxy', () =>
      setUp((data) => {
    ...
    it('requires a directory path for proxy installs', () =>
      setUp((data) => {
    ...
    it('re-uses an existing extension directory', () =>
      setUp((data) => {
        return fs
          .mkdir(path.join(data.profile.extensionsDir))
          .then(() => installBasicExt(data))
          .then(() => fs.stat(data.profile.extensionsDir));
      }));

    it('checks for an empty extensionsDir', () =>
      setUp((data) => {
