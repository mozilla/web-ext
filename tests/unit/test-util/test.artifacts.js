...
describe('prepareArtifactsDir', () => {
  ...
  it('ensures the path is really a directory', () =>
    withTempDir((tmpDir) => {
      ...
      return fs
        .writeFile(someFile, 'some content')
  ...
  it('resolves with the artifacts dir', () =>
    withTempDir((tmpDir) => {
      const artifactsDir = path.join(tmpDir.path(), 'artifacts');
      return prepareArtifactsDir(artifactsDir).then((resolvedDir) => {
        assert.equal(resolvedDir, artifactsDir);
      });
    }));

  it('throws an UsageError when it lacks permissions to stat the directory', function () {
  ...
  it('throws error when directory exists but lacks writing permissions', function () {
    return withTempDir((tmpDir) => {
      if (process.platform === 'win32') {
        this.skip();
        return;
  ...
  it('throws error when creating a folder if lacks writing permissions', function () {
    return withTempDir((tmpDir) => {
      if (process.platform === 'win32') {
        this.skip();
        return;
  ...
  it('creates the artifacts dir successfully if the parent dir does not exist', () =>
    withTempDir((tmpDir) => {
      ...
      return prepareArtifactsDir(tmpPath).then((resolvedDir) => {
