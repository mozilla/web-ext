...
describe('util.isDirectory', () => {
  it('resolves true for a directory', () =>
    withTempDir((tmpDir) => {
      return isDirectory(tmpDir.path()).then((dirExists) => {
        assert.equal(dirExists, true);
      });
    }));

  it('resolves false for non-existent paths', () => {
    return isDirectory('/dev/null/not-a-real-path-at-all').then((dirExists) => {
      assert.equal(dirExists, false);
    });
  });

  it('resolves false for non-directory paths', () =>
    withTempDir((tmpDir) => {
  ...
  it('resolves false for incomplete directory paths', () =>
    withTempDir((tmpDir) => {
