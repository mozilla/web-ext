...
describe('util/extension-runners/firefox-desktop', () => {
  it('installs and runs the extension', async () => {
    ...
    sinon.assert.calledWith(params.firefoxApp.run, sinon.match(fakeProfile));
  });
  ...
  it('runs extension in correct port', async () => {
    ...
    sinon.assert.calledWith(params.firefoxClient, sinon.match({ port: 6008 }));
  });
  ...
  it('raise an error on addonId missing from installTemporaryAddon result', async () => {
    const { params } = prepareExtensionRunnerParams({
      fakeRemoteFirefox: {
        installTemporaryAddon: sinon.spy(() =>
  ...
  it('raises an Error when exit method is called on a non-started runner', async () => {
    const { params } = prepareExtensionRunnerParams();
  ...
  it('resolves to an array of WebExtError if the extension is not reloadable', async () => {
    ...
    await runnerInstance
      .reloadExtensionBySourceDir('/non-existent/source-dir')
      .then((results) => {
  ...
  it('resolves to an AllExtensionsReloadError if any extension fails to reload', async () => {
    const { params, remoteFirefox } = prepareExtensionRunnerParams({
      fakeRemoteFirefox: {
    ...
    await runnerInstance.reloadAllExtensions().then((results) => {
      const error = results[0].reloadError;
      assert.equal(error instanceof WebExtError, true);
